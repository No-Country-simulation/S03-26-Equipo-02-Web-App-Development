...
const ChatWindow = ({ contactId }: { contactId: number | null }) => {
  ...
  // Generate random chat history for each contact
  const chatMessages = useMemo(() => {
    if (!contact) return [];
    // Use contact ID as seed for reproducible random messages
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50/30 h-full animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b bg-white backdrop-blur-md sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="relative ml-4">
            <button
              ...
              className="flex items-center gap-2 text-[11px] font-bold px-3 py-1.5 rounded-lg border border-slate-100 bg-slate-50 hover:bg-slate-100 transition-all cursor-pointer group"
            >
            ...
            {open && (
              <div className="absolute top-10 left-0 w-36 bg-white border border-slate-100 rounded-xl shadow-xl z-[100] p-1.5 animate-in zoom-in-95 duration-200">
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-[11px] font-bold text-slate-600 hover:bg-[#0D9488]/10 hover:text-[#0D9488] rounded-lg transition-colors"
                >
                  Prospecto
                </button>
                <button
        ...
        <button className="p-2 hover:bg-slate-50 rounded-lg transition-colors text-slate-400 hover:text-slate-600 cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 custom-scrollbar">
        <div className="space-y-8 max-w-4xl mx-auto">
          <div className="flex items-center justify-center">
            <p className="px-4 py-1 bg-slate-100/80 rounded-full text-[10px] text-slate-500 font-extrabold uppercase tracking-widest">
              Hoy
            </p>
          </div>
            ...
            >
              <div className="flex flex-col max-w-[75%]">
                {msg.sender === "other" && msg.name && (
                  <p className="text-[10px] font-bold text-slate-400 mb-1.5 ml-1 uppercase tracking-wider">{msg.name}</p>
                )}

                <div
                  className={`rounded-2xl px-5 py-3.5 text-sm shadow-sm transition-all hover:shadow-md ${
                    msg.sender === "me"
                ...
                >
                  ...
                  <span
                    className={`block text-[9px] font-bold mt-2.5 uppercase tracking-tighter ${
                      msg.sender === "me"
                        ? "text-[#CCFBF1] text-right"
                        : "text-slate-400"
                    }`}
                  >
                    {msg.time}
      ...
      <div className="bg-white border-t px-6 py-4 flex items-center gap-4 sticky bottom-0">
        <div className="flex-1 relative">
           <input
            type="text"
            placeholder={`Escribe un mensaje a ${contact.name}...`}
            className="w-full h-12 bg-slate-50/50 border border-slate-100 rounded-xl px-5 py-2 text-sm font-semibold placeholder:text-slate-400 outline-none focus:ring-4 focus:ring-[#0D9488]/10 focus:bg-white transition-all"
          />
        </div>
