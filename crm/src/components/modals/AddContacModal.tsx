...
interface AddContactModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContactCreated: (contact: ApiContact) => void;
}
  ...
  const handleAddContact = async (): Promise<void> => {
    try {
      ...
      const response = await fetch(
        "https://s03-26-equipo-02-web-app-development.onrender.com/contacts",
        {
          method: "POST",
          headers: {
      ...
      // ⚠️ importante: tu API devuelve { data: contact }
      const newContact: ApiContact = result.data;

      // 🔥 esto actualiza la tabla sin recargar
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                    setFormData({
                      ...formData,
                      status: e.target.value as ContactFormData["status"],
                    })
                  }
