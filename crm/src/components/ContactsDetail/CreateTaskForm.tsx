...
const CreateTaskForm = ({
  ...
  const fetchTasks = async () => {
    try {
      ...
      const tasksData: ApiTask[] = Array.isArray(response)
        ? response
        : response.data || [];

      const filteredTasks = tasksData.filter((t: ApiTask) => {
        return t.contact?.id === contactId;
      });

      const formatted: FormattedTask[] = filteredTasks.map((t: ApiTask) => {
  ...
  const handleAddTask = async () => {
    if (!taskForm.title || !taskForm.date || !taskForm.priority || loading)
      return;
    setLoading(true);
    try {
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)] p-8">
        ...
        <div className="space-y-5">
          ...
          <button
            ...
            disabled={
              loading || !taskForm.title || !taskForm.date || !taskForm.priority
            }
        ...
        {tasks.length > 0 ? (
          tasks.map((task) => (
            <div
              key={task.id}
              className={cn(
                "bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.03)] p-6 transition-all hover:shadow-md group",
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4 flex-1">
                  ...
                  <div className="flex flex-col flex-1">
                    <h4
                      className={cn(
                        "text-[15px] font-bold transition-colors group-hover:text-[#0D9488]",
                        task.completed
                  ...
                  <span
                    className={cn(
                      ...
                      task.completed
                        ...
                        : `text-white ${getPriorityColor(task.priority)}`,
                    )}
                  >
                    {task.priority}
                  </span>
          ...
          <p className="text-slate-400 text-sm font-bold text-center py-6">
            No hay tareas asociadas a este contacto.
          </p>
        )}
      </div>
    </div>
